...
from data.types import Course, CourseAPI, CourseFilters, courseFromAPI
...
def fetch_courses(filters: CourseFilters) -> List[Course]:
    ...
    for course in first_payload.get("posts", []):
        if isinstance(course, dict) and course.get("ID") is not None:
            courseInstance = courseFromAPI(course)
            if courseInstance:
                courses_by_id[courseInstance.id] = courseInstance
    ...
    return sorted(courses_by_id.values(), key=lambda c: c.date, reverse=True)
